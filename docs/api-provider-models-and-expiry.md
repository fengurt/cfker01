...
| Perplexity | `GET https://api.perplexity.ai/v1/models` | The reference omits an authorization field, but the live endpoint returned `401` without a valid Bearer key on 2026-08-12; the monitor therefore authenticates | Not documented; key values are one-time reveal and can be revoked/rotated | Credit balance is documented as a console feature; no supported credit-expiry API was found. |
...
`GET /v1/models` has no authorization field in the current reference. However, a live request without a valid Bearer key returned `401` on the research date, so production discovery must authenticate and treat the documented omission as insufficient evidence of anonymous access. It returns `object: "list"` and `data[]`; model objects contain `id`, `object`, `created`, and `owned_by`. The list currently targets models usable by the Agent API. [Perplexity List Models](https://docs.perplexity.ai/api-reference/models-get), [Perplexity changelog](https://docs.perplexity.ai/docs/resources/changelog)
...
7. For Perplexity, use the inference key for model discovery until the provider explicitly documents and serves anonymous access consistently; a bad key can therefore block both model discovery and inference health.
