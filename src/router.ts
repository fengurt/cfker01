...
import { bootstrapAdmin, clearAdminSession, createAdminSession, readAdminSession, requireAdminToken } from "./lib/auth";
...
export async function handleRequest(
  ...
  if (pathname === "/admin/logout" && request.method === "POST") return clearAdminSession(request);
  ...
  if (pathname === "/api/v1/projects" || pathname.startsWith("/api/v1/projects/")) return handleProjectsApi(request, env, ctx);

  if (pathname === "/admin/projects" || pathname.startsWith("/admin/projects/")) return handleAdminProjects(request, env, ctx);
  if (pathname.startsWith("/admin/resources/")) return handleAdminResourceOps(request,env,ctx);
  if (pathname === "/admin/servers" || pathname.startsWith("/admin/servers/")) return handleAdminServers(request,env,ctx);
  if (pathname === "/admin/benchmarks" || pathname.startsWith("/admin/benchmarks/")) return handleAdminBenchmarks(request,env,ctx);
  if (pathname === "/admin/repository-reviews" || pathname.startsWith("/admin/repository-reviews/")) return handleAdminRepositoryReviews(request,env,ctx);
  if (pathname === "/admin/assets" || pathname.startsWith("/admin/assets/")) return handleAdminAssets(request,env,ctx);
  if (pathname === "/admin/discovery-runs" || pathname.startsWith("/admin/discovery-runs/")) return handleAdminDiscovery(request,env,ctx);
  if (pathname === "/admin/resource-links" || pathname.startsWith("/admin/resource-links/")) return handleAdminLinks(request,env,ctx);
  if (pathname === "/admin/monitor" || pathname.startsWith("/admin/monitor/")) return handleAdminMonitor(request,env,ctx);

  if (pathname === "/api/v1/catalog" || pathname.startsWith("/api/v1/catalog/") || pathname === "/api/v1/articles" || pathname.startsWith("/api/v1/articles/")) {
