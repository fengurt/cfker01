...
export async function handleAdminServers(request:Request,env:Env,ctx:ExecutionContext){
  ...
  if(request.method==="GET"){
    ...
    const [servers,deployments,runtimeAssets,dnsAssets,repositoryAssets,billingAssets,billingRun]=await Promise.all([
      ...
      env.MGMT_DB.prepare(`SELECT * FROM discovered_assets WHERE provider IN ('docker','tencent') AND kind IN ('cvm','lighthouse','container','compose_project','server_runtime','runtime_container','runtime_service','tat_agent') AND status!='stale' AND server_id IS NOT NULL ORDER BY server_id,kind,name`).all<Record<string,unknown>>(),
    ...
    return Response.json({data:serverRows.map(server=>{const runtime=(runtimeByServer.get(String(server.id))??[]).map(value=>attachRepository(value,repositories)),host=runtime.find(value=>value.kind==="server_runtime")??null,cloudAsset=runtime.find(value=>["cvm","lighthouse"].includes(String(value.kind)))??null,dns=dnsByServer.get(String(server.id))??[],billing=billingByServer.get(String(server.id))??[];return{...server,region:server.region??cloudAsset?.region??null,effective_status:effectiveStatus(server),is_stale:isStale(server),runtime_coverage:runtimeCoverage(runtime),runtime_assets:runtime,runtime_projects:aggregateRuntimeProjects(runtime,host),dns_records:dns,billing:billingSummaryForServer(billing),deployments:(byServer.get(String(server.id))??[]).map(deployment=>({...deployment,effective_health_status:deploymentHealth(deployment)}))};}),meta:{billing:billingSummary}});
  }
...
function parseMetadata(value:unknown){if(value&&typeof value==="object")return value as Record<string,unknown>;try{return JSON.parse(String(value));}catch{return{};}}
...
function billingSummaryForServer(rows:Record<string,unknown>[]){const records=rows.filter(row=>row.kind==="billing_resource").map(row=>({id:row.id,name:row.name,lastSeenAt:row.last_seen_at,metadata:parseMetadata(row.metadata)}));const costs=records.map(row=>Number(row.metadata?.realTotalCost??row.metadata?.totalCost)).filter(value=>Number.isFinite(value));return{status:records.length?"observed":"unavailable",month:records[0]?.metadata?.month??null,monthlyCostCNY:costs.length?Number(costs.reduce((sum,value)=>sum+value,0).toFixed(2)):null,resourceCount:records.length,records};}
