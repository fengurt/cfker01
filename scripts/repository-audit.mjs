...
await writeFile(join(cacheDir,"summary.json"),JSON.stringify({generatedAt:report.generatedAt,count:report.count,changed,cacheHits,dirty:repositories.filter(x=>x.dirty).length,missingRemote:repositories.filter(x=>!x.repositoryUrl).length,githubMatched:repositories.filter(x=>x.githubMetadata).length},null,2));
if(upload)await uploadAll(repositories.filter(item=>full||!item.cacheHit));
console.log(JSON.stringify({count:report.count,changed,cacheHits,dirty:repositories.filter(x=>x.dirty).length,missingRemote:repositories.filter(x=>!x.repositoryUrl).length,output:join(cacheDir,"latest.json")},null,2));
...
async function topFiles(directory){try{return(await readdir(directory,{withFileTypes:true})).filter(x=>x.name!==".git"&&!ignored.has(x.name)&&!sensitive.test(x.name)).map(x=>x.isDirectory()?`${x.name}/`:x.name).sort();}catch{return[];}}
