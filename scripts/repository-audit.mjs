...
import { localPath, localScanRoots } from "./lib/local-scan-roots.mjs";

const exec=promisify(execFile),args=new Set(process.argv.slice(2));
const roots=localScanRoots({env:{...process.env,LOCAL_SCAN_ROOTS:process.env.REPOSITORY_SCAN_ROOTS||process.env.LOCAL_SCAN_ROOTS,LOCAL_SCAN_ROOT:process.env.REPOSITORY_SCAN_ROOT||process.env.LOCAL_SCAN_ROOT}});
...
const gitDirs=[];for(const root of roots)await walk(root,gitDirs,root);
const local=[];for(const {directory,root} of gitDirs)local.push(await inspect(directory,root));
const merged=new Map();for(const item of local){const existing=merged.get(item.canonicalKey);if(existing){existing.localPaths.push(...item.localPaths);existing.skillPaths=[...new Set([...(existing.skillPaths||[]),...(item.skillPaths||[])])];existing.dirty=existing.dirty||item.dirty;continue;}merged.set(item.canonicalKey,item);}
...
const report={version:"repository-audit-v1",generatedAt:new Date().toISOString(),roots,count:repositories.length,changed,cacheHits,repositories};
...
async function walk(directory,out,root){let entries;try{entries=await readdir(directory,{withFileTypes:true});}catch{return;}if(entries.some(x=>x.isDirectory()&&x.name===".git")){out.push({directory,root});return;}for(const entry of entries)if(entry.isDirectory()&&!ignored.has(entry.name))await walk(join(directory,entry.name),out,root);}
...
function buildDossier(item,old){const value={identity:{key:item.canonicalKey,name:item.name,remote:item.repositoryUrl,localPaths:item.localPaths},git:{head:item.headSha,branch:item.branch,dirty:item.dirty,ahead:item.ahead,behind:item.behind,lastCommitAt:item.lastCommitAt},github:{defaultBranch:item.defaultBranch,pushedAt:item.pushedAt,visibility:item.visibility,archived:item.archived,fork:item.fork,ciStatus:item.ciStatus,release:item.releaseName,topics:item.topics},project:{description:item.description,languages:item.languages,frameworks:item.frameworks,skillPaths:item.skillPaths,entrypoints:item.entrypoints,topLevel:item.topLevel,readmeExcerpt:item.readmeExcerpt},hygiene:item.hygiene,deployment:{fingerprint:item.deploymentFingerprint,status:item.deploymentStatus},change:old?{previousFingerprint:old.fingerprint,headChanged:old.headSha!==item.headSha,manifestsChanged:old.manifestHash!==item.manifestHash,lockfileChanged:old.lockfileHash!==item.lockfileHash}: {firstReview:true}};return sanitize(JSON.stringify(value,null,2)).slice(0,8192);}
