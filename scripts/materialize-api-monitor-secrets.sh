...
for key in DOUBAO_ARK_CREDENTIAL_EXPIRES_AT MINIMAX_API_CREDENTIAL_EXPIRES_AT MINIMAX_CODING_PLAN_CREDENTIAL_EXPIRES_AT MINIMAX_CODING_PLAN_SUBSCRIPTION_EXPIRES_AT MINIMAX_CODING_PLAN_QUOTA_RESETS_AT OPENAI_CREDENTIAL_EXPIRES_AT PERPLEXITY_CREDENTIAL_EXPIRES_AT MOONSHOT_CREDENTIAL_EXPIRES_AT GEMINI_CREDENTIAL_EXPIRES_AT; do
  value="${!key:-}"
  [[ -z "$value" ]] || printf '%s=%s\n' "$key" "$value" >> "$TARGET"
done
chmod 600 "$TARGET"
