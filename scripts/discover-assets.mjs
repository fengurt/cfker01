...
if command -v docker >/dev/null 2>&1; then
  ...
  docker stats --no-stream --format 'STAT\\t{{.ID}}\\t{{.CPUPerc}}\\t{{.MemUsage}}\\t{{.MemPerc}}' 2>/dev/null || true
fi
...
async function dockerApiContainers(){const base=new URL(process.env.DOCKER_API_URL),list=await httpJson(base,"/containers/json?all=1");return Promise.all(list.map(item=>httpJson(base,`/containers/${encodeURIComponent(item.Id)}/json`)));}
