...
const roots = localScanRoots({ argv: process.argv.slice(2) });
...
const result = { generatedAt: scannedAt, scanRoots: roots, count: list.length, resources: list };
await writeFile(output, `${JSON.stringify(result, null, 2)}\n`);
console.log(`Scanned ${roots.join(", ")}: ${list.length} resources written to ${output}`);
...
async function walk(directory, root) {
  ...
  if (git) await addResource(directory, ".git", "repository", root);
  for (const item of items) {
    if (item.isDirectory()) { if (!ignored.has(item.name)) await walk(join(directory, item.name), root); continue; }
    if (item.isFile() && markers.has(item.name)) await addResource(directory, item.name, markerType(item.name), root);
  }
}

async function addResource(directory, marker, type, root) {
  const markerPath = join(directory, marker);
  const resourcePath = marker === "SKILL.md" ? markerPath : directory;
  const location = localPath(root, resourcePath);
  const sourceRef = location.absolutePath;
  const key = sourceRef;
  const existing = resources.get(key) || { sourceRef, scanRoot: location.scanRoot, relativePath: location.relativePath, name: basename(directory), description: null, resourceTypes: new Set(), markers: [], languages: new Set(), frameworks: new Set(), repositoryUrl: null, discoveredAt: new Date().toISOString(), sourceUpdatedAt: null, entrypoints: new Set(), workspaces: [] };
